...
import Topbar from "@/components/dashboard/Topbar";
import { toast } from "sonner";
import { FaUser, FaCalendarAlt, FaEdit, FaTrash, FaCheckCircle, FaTimesCircle, FaFolder } from "react-icons/fa";
...
export default function BlogList() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`/api/blogs/delete?id=${id}`, { method: "DELETE" });
      const data = await res.json();
      if (data.success) {
        toast.success("Blog deleted successfully");
        fetchBlogs();
                ...
                <div key={blog._id} className="col-md-6 col-lg-4">
                  ...
                  >
                    ...
                    <div style={{ padding: "15px", flexGrow: 1, display: "flex", flexDirection: "column" }}>
                      <h5 style={{ fontWeight: "600", marginBottom: "10px" }}>{blog.title}</h5>
                      <div style={{ fontSize: "0.85rem", color: "#555", marginBottom: "10px", display: "flex", flexWrap: "wrap", gap: "10px" }}>
                        {blog.category && (
                      ...
                      <p style={{ fontSize: "0.85rem", color: "#666", flexGrow: 1 }}>
                      ...
                      <div style={{ fontSize: "0.8rem", color: "#777", display: "flex", justifyContent: "space-between", marginTop: "10px", alignItems: "center" }}>
                        <span style={{ display: "flex", alignItems: "center", gap: "5px" }}>
                          <FaUser /> {blog.authorName || "Admin"}
                        </span>
                        <span style={{ display: "flex", alignItems: "center", gap: "5px" }}>
                          <FaCalendarAlt /> {new Date(blog.publishDate).toLocaleDateString()}
                        </span>
                        <span style={{ display: "flex", alignItems: "center", gap: "5px" }}>
                          {blog.status === "published" ? (
                            <span style={{ color: "green", display: "flex", alignItems: "center", gap: "3px" }}>
