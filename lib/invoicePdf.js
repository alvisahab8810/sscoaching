...
export function generateInvoicePdf(invoice) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const cellPad = 10; // left inset so cell content doesn't hug the table border
      ...
      doc.text("#",           xNum + cellPad,  tableTop + 8, { width: colNum - cellPad,  characterSpacing: 0.4 });
      doc.text("DESCRIPTION", xDesc + cellPad, tableTop + 8, { width: colDesc - cellPad, characterSpacing: 0.4 });
      doc.text("QTY",         xQty,  tableTop + 8, { width: colQty,  characterSpacing: 0.4 });
      doc.text("RATE",        xRate, tableTop + 8, { width: colRate - 8, characterSpacing: 0.4, align: "right" });
      doc.text("AMOUNT",      xAmt,  tableTop + 8, { width: colAmt - 8,  characterSpacing: 0.4, align: "right" });

      let rowY = tableTop + 24;

      const descW = colDesc - cellPad - 8;
      ...
      fill(doc, C.dark);
      doc.font(F).fontSize(9).text("1", xNum + cellPad, rowMidY, { width: colNum - cellPad });

      fill(doc, C.dark);
      doc.font(FSB).fontSize(10).text(courseTitle, xDesc + cellPad, rowY + 8, { width: descW, lineGap: 1 });
      if (courseSub) {
        fill(doc, C.gray);
        doc.font(F).fontSize(8).text(courseSub, xDesc + cellPad, rowY + 8 + titleH + 4, { width: descW });
      }

      fill(doc, C.dark);
      doc.font(F).fontSize(9).text("1", xQty, rowMidY, { width: colQty });
      doc.text(INR(invoice.subtotal), xRate, rowMidY, { width: colRate - 8, align: "right" });
      doc.font(FSB).text(INR(invoice.subtotal), xAmt, rowMidY, { width: colAmt - 8, align: "right" });
      ...
      if (invoice.discount > 0) {
        ...
        fill(doc, C.green);
        doc.font(F).fontSize(9).text(label, xDesc + cellPad, rowY + 8, { width: colDesc + colQty + colRate - cellPad - 8 });
        doc.font(FSB).text(`− ${INR(invoice.discount)}`, xAmt, rowY + 8, { width: colAmt - 8, align: "right" });
        rowY += 24;
        stroke(doc, C.border);
        doc.moveTo(tableX, rowY).lineTo(tableX + innerW, rowY).lineWidth(0.5).stroke();
      }
      ...
      if (invoice.tax > 0) {
        fill(doc, C.dark);
        doc.font(F).fontSize(9).text("Tax (GST)", xDesc + cellPad, rowY + 8, { width: colDesc + colQty + colRate - cellPad - 8 });
        doc.font(FSB).text(INR(invoice.tax), xAmt, rowY + 8, { width: colAmt - 8, align: "right" });
        rowY += 24;
        stroke(doc, C.border);
        doc.moveTo(tableX, rowY).lineTo(tableX + innerW, rowY).lineWidth(0.5).stroke();
      }
      ...
      fill(doc, C.dark);
      doc.font(FB).fontSize(10.5).text("Total", xDesc + cellPad, rowY + 10, { width: colDesc + colQty + colRate - cellPad - 8 });
      fill(doc, C.purple);
      doc.font(FB).fontSize(13).text(INR(invoice.total), xAmt, rowY + 8, { width: colAmt - 8, align: "right" });
      rowY += totalH;
      ...
      /* GST note — the course price is GST-inclusive (no separate tax is
         charged on top), so this is a disclosure line only: it shows the
         embedded CGST/SGST split as percentages, not an added rupee amount. */
      if (invoice.total > 0) {
        fill(doc, C.gray);
        doc.font(F).fontSize(8).text("Inclusive of GST @ 18% (CGST 9% + SGST 9%)", tableX, cursorY, { width: innerW });
        cursorY += 14;
      }
...
<div class="card">
  ...
  ${invoice.total > 0 ? `<div style="font-size:10px;color:#8A8FA3;margin:-12px 0 20px;">Inclusive of GST @ 18% (CGST 9% + SGST 9%)</div>` : ""}
