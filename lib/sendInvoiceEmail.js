...
function createTransporter() {
  return nodemailer.createTransport({
    ...
    secure: false,
    auth: {
      user: "info@viralon.in",       // ← change to SS Coaching email when ready
...
<body style="margin:0;padding:0;background:#f4f5ff;font-family:'Segoe UI',Arial,sans-serif;">
  <div style="max-width:560px;margin:32px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(68,65,229,0.1);">
    ...
    <!-- Header -->
    <div style="background:linear-gradient(135deg,#0f1f3d,#1b1870);padding:32px 36px;text-align:center;">
      <div style="width:52px;height:52px;background:linear-gradient(135deg,#4441e5,#6b68ff);border-radius:12px;display:inline-flex;align-items:center;justify-content:center;color:#fff;font-size:22px;font-weight:900;margin-bottom:12px;">SS</div>
      <div style="color:#fff;font-size:22px;font-weight:900;">SS Coaching</div>
      <div style="color:rgba(255,255,255,0.5);font-size:12px;margin-top:4px;">Rise From Failure • Estd. 2001</div>
    </div>
...
export async function sendInvoiceEmail({ invoice, studentEmail }) {
  ...
  try {
    ...
    await transporter.sendMail({
      from: `"SS Coaching" <info@viralon.in>`,
      to:   email,
